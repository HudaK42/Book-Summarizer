...
#     if login_btn:
#         try:
#             res = requests.post(f"{BACKEND_URL}/login", json={"email": email, "password": password}, timeout=10)
#             data = res.json()
#             if data.get("status") == "success":
#                 st.success("Login successful!")
#                 st.session_state.logged_in = True
#                 st.session_state.user_id = data.get("user_id")  # important
#                 st.session_state.user_name = data.get("name")
#                 st.session_state.role = data.get("role")
#                 st.session_state.page = "dashboard"
                
#                 # ✅ Force rerun for latest Streamlit
...
#                 st.session_state["__rerun"] = True  # Force rerun
#             else:
#                 st.error(data.get("message"))
#         except Exception as e:
#             st.error(f"Error connecting to backend: {e}")
